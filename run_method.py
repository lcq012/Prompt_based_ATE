...
def run_method(dataset: str, hparams_path: str=None, seed=42, train=None, dev=None, test=None, unlabeled=None):
    ...
    training_args.do_train = train is not None                   # True
    training_args.do_eval = dev is not None                      # False
    training_args.do_predict = test is not None                  # True
    set_seed(seed)
    training_args.seed = seed
    # 它可以自动优化模型的训练过程，使其在多个GPU或TPU上运行时更快
    accelerator = Accelerator()
    set_up_logging(accelerator)
    training_args.max_train_steps = args.max_train_steps
    # training_args.num_train_epochs = int(args.num_train_epochs)
    ...
    seed:42
    test:None
    train、unlabeled、dev、test:Dataset
    """
    with track('Total Run'):
        ############################ Load Data ####################################
        #           将数据集以train、dev、test、unlabeled的形式加载进来
        ...
        with track('Pre-process'):
            ...
            with track('----Init tokenizer'):
                tokenizer = AutoTokenizer.from_pretrained(
                    ...
                    use_fast=True,
                    # add_prefix_space=True,                                      # add_prefix_space 是一个可选参数，用于指示 tokenizer 是否应该在输入的开头添加一个空格
                    **proxy
                )
        ...
        ##                      如果没有few-shot则是掩码的模型
        ##                      如果是few-shot则是正常的bert
        with track('Load Model'):
            if args.few_shot:
                    model = BertForPatternMaskedLM.from_pretrained(
        ...
        ################################# Fine-Tune - Phase 2 (Optional) #################################
                if args.few_shot and args.tr_phase_2:
                    ...
                    train_conf = {"lm": args.tr_phase_2_lm, "steps": args.tr_phase_2_steps,
                        "lr": args.tr_phase_2_lr, "label_loss": args.tr_phase_2_label_loss, "alpha": args.alpha}
