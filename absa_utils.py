...
class AbsaPVP:
    ...
    def __init__(
        ...
        """
        'ADJ': 'adjective 形容词', 'ADP': 'adposition 介词', 'ADV': 'adverb 副词',
        'AUX': 'auxiliary verb 助动词','CONJ': 'coordinating conjunction 并列连词',
        'DET': 'determiner 限定词', 'INTJ': 'interjection 感叹词', 'NOUN': 'noun 名词',
        'NUM': 'numeral 数词', 'PART': 'particle 助词', 'PRON': 'pronoun 代词',
        'PROPN': 'proper noun 专有名词', 'PUNCT': 'punctuation 标点符号',
        'SCONJ': 'subordinating conjunction 从属连词',  'SYM': 'symbol 符号',
        'VERB': 'verb 动词',    'X': 'other'
        """
    ...
    def preprocess(self, examples: Dict[str, List[Any]], split: str) -> Dict[str, List[Any]]:
            ...
            for text, tokens, tags, ace_pred in zip(examples['text'], examples['tokens'], examples['tags'], ace_preds):
                ...
                # extract_aspect_candidates:抽取句子中的候选属性词
                # phrase_texts:代表可能的属性词片段
                # phrase_idx:代表对应片段的索引值（起始，终止）
                # pred_groups:预测的结果
                # cls_labels:对应的真实结果
                ...
                # P_x_text:代表sentence+prompt的形式
                # phrase_input_ids:输入句子的id号
                # phrase_token_type_ids:输入句子对应的type_id号
    ...
    def get_parts(self, text: str, asp_candidate: str, mask_token_label=None) -> FilledPattern:
        ...
        #     return [text, ". So, does the review focus on", asp_candidate, "?", mask_token]
            ...
            # res14 category
            return [text, ". So, aspects categories are in the following categories: {ambience, anecdotes/miscellaneous, food, price, service } and does the review in the previous sentence focus on", asp_candidate, "?", mask_token]
